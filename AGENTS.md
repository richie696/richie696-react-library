...
Before changing public APIs or application architecture, read the [React 工程规范索引](docs/REACT_ENGINEERING_STANDARD.md) and the relevant generic UI/UE, coding or project-skeleton document. For this repository's published APIs, also read [底座 API 使用指南](docs/RICHIE_FOUNDATION_USAGE.md) and verify the actual package exports. Apply the `modern-react-ui-design` skill for React application structure, data/state ownership and SCSS skin work.
