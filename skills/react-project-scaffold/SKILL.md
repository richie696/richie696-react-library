...
This skill creates a new product repository, not a feature inside an existing one. Read the maintained [project skeleton standard](../../docs/REACT_PROJECT_SKELETON.md) before choosing the profile. The supplied generator currently supports a client-side Vite SPA with React 19, TypeScript 6, strict types, Hooks lint, and SCSS light/dark skin. It does not generate SSR/Next.js, a router, API, a UI kit, or the Richie foundation package. If the requested platform differs, do not mislabel this SPA template as a match.

The canonical generator is [create_react_project.py](../../scripts/create_react_project.py); its source template is `templates/react-spa` in this repository. Resolve the repository root from this skill's directory. Inspect the exact destination first. The parent directory must exist and the destination must not exist. Run a preview, then generate:

```bash
python3 /path/to/richie696-react-library/scripts/create_react_project.py --name my-react-app --output /absolute/path/my-react-app --dry-run
python3 /path/to/richie696-react-library/scripts/create_react_project.py --name my-react-app --output /absolute/path/my-react-app
```
